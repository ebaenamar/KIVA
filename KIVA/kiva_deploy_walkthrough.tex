...
\begin{itemize}
    ...
    \item \textcolor{kivared}{Not supported on Windows}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Not needed:} Google OAuth (disabled by default)
\end{itemize}
...
\node[stepbox] (s1) {\faGithub\ \texttt{git clone https://github.com/sensein/riverst.git}};
\node[stepbox, below=of s1] (s2) {\faPython\ \texttt{conda create -n riverst python=3.11} \& activate};
\node[stepbox, below=of s2] (s3) {\faKey\ \texttt{cp env.example .env} -- Add your OpenAI key};
\node[stepbox, below=of s3] (s4) {\faNpm\ \texttt{pip install -r requirements.txt} \& \texttt{npm install}};
\node[stepbox, below=of s4] (s5) {\faPlay\ \texttt{python main.py} + \texttt{npm run dev}};
